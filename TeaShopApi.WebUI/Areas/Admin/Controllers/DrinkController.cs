...
namespace TeaShopApi.WebUI.Areas.Admin.Controllers
{
	...
	public class DrinkController : Controller
	{
        ...
        public async Task<IActionResult> CreateDrink(CreateDrinkDto createDrinkDto)
		{
			...
			var responseMessage = await client.PostAsync("https://localhost:7028/api/Drinks", content);
			if (responseMessage.IsSuccessStatusCode)
			{
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responMessage = await client.DeleteAsync("https://localhost:7028/api/Drinks?id="+id);
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.GetAsync("https://localhost:7028/api/Drinks/" + id);
		...
		public async Task<IActionResult> UpdateDrink(UpdateDrinkDto updateDrinkDto)
		{
			...
			var responseMessage = await client.PutAsync("https://localhost:7028/api/Drinks/", stringContent);
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
